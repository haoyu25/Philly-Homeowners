...
ggplot(xgb_full_pred4, aes(x = .pred4, fill = as.factor(exemption))) + 
  geom_density() +
  facet_grid(exemption ~ .) +
...
```{r}
xgb_full_pred4$exemption.pred4 <- factor(as.character(xgb_full_pred4$exemption.pred4), levels = c("0", "1"))
xgb_full_pred4$exemption       <- factor(as.character(xgb_full_pred4$exemption),       levels = c("0", "1"))

caret::confusionMatrix(xgb_full_pred4$exemption.pred4, xgb_full_pred4$exemption, 
                       positive = "1")
```
...
xgb_full_pred4 <- xgb_full_pred4 %>%
  mutate(exemption.pred0.5 = ifelse(.pred4 >= 0.5, "1", "0"),
         exemption.pred0.6 = ifelse(.pred4 >= 0.5, "1", "0"),
         exemption.pred0.7 = ifelse(.pred4 >= 0.5, "1", "0"),
         exemption.pred0.8 = ifelse(.pred4 >= 0.5, "1", "0"),
         exemption.pred0.9 = ifelse(.pred4 >= 0.5, "1", "0"))
```


```{r}
fwrite(xgb_full_pred4, "xgb_full_pred.csv")
```
...
```{r}
xgb_full_pred_sf <- xgb_full_pred4 %>%
  mutate(residual1 = as.numeric(exemption) - .pred1,
         residual2 = as.numeric(exemption) - .pred2,
         residual3 = as.numeric(exemption) - .pred3,
         residual4 = as.numeric(exemption) - .pred4)
```
...
# 转为 long format 更好对比
residuals_long <- xgb_full_pred_sf %>%
  st_drop_geometry()%>%
  dplyr::select(exemption, residual1, residual2, residual3, residual4) %>%
  pivot_longer(cols = starts_with("residual"),
               names_to = "model",
               values_to = "residual")

# 计算 MAE、RMSE
residual_metrics <- residuals_long %>%
  group_by(model) %>%
  summarise(
...
ggplot(residuals_long, aes(x = residual, fill = model)) +
  ...
  labs(title = "Residual Distribution Comparison")
```

```{r}
sf_data <- xgb_full_pred_sf %>%
...
max_val <- max(abs(c(xgb_full_pred_sf$residual1,
                     xgb_full_pred_sf$residual2,
                     xgb_full_pred_sf$residual3,
                     xgb_full_pred_sf$residual4)), na.rm = TRUE)

max_val
range(xgb_full_pred_sf$residual1, na.rm = TRUE)
```


```{r}
tm_shape(sf_data) +
  tm_fill("residual2", palette = colors6, 
          style = "cont",  # 确保数据是连续型的
          breaks = c(-max_val, seq(-max_val, max_val, length.out = 6), max_val),
          title = "Residual") +
  tm_borders() +
  tm_layout(main.title = "Residuals - Model 2")
```
...
```{r}
# Residual 2
ggplot(xgb_full_pred_sf) +
  geom_sf(aes(fill = balance_rate), color = NA) +
  scale_fill_gradientn(colors = colors6,
                       name = "Residual") +
...
# Residual 3
ggplot(sf_data) +
  geom_sf(aes(fill = residual3), color = NA) +
  scale_fill_gradientn(colors = colors6,
