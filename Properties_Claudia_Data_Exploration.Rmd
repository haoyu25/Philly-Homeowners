...
sum(is.na(combined$com_potential_bin))
combined$avg_balance
combined <- combined %>% mutate(com_potential_final = ifelse(is.na(com_potential_bin), 0, com_potential_bin))
combined <- combined %>% mutate(avg_balance_final = ifelse(is.na(avg_balance), 0, avg_balance))

properties_model <- combined
set.seed(3456)
...
churnreg1 <- glm(exemption ~ .,
                 data=churnTrain %>% 
                   dplyr::select(market_value, total_area, zip_code, geographic_ward, is_residential, same_address, likely_loop, is_deep, avg_balance_final, com_potential_final, exemption),
