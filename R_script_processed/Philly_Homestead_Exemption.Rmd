...
properties$has_code <- str_detect(data, "Z50")
print(data)
any(grepl("Z75", as.matrix(properties), fixed = TRUE))
rows_with_Z50 <- apply(properties, 1, function(row) any(grepl("Z50", row, fixed = TRUE)))
properties[rows_with_Z50, ]
...
An classification type XGBoost was then defined with 200 trees, while the number of variables randomly sampled at each split and the minimum number of observation in a terminal node are tuned with with the help of a hyperparameter grid search.
...
The grid search is conducted with the best model selected based on having the lowest false negative rate.
```{r}
## metrics across grid
...
The model’s sensitivity—its ability to correctly identify current exemption holders—is high and consistent across most demographic groups, with rates around 86% and false negative rates (misses) around 14%. 
...
# Target Outreach
The target outreach group are the false positives. At a 0.5 threshold, this amounts to 75,708 houses.

## Neighborhood Demographic Profiles
In terms of demographic across census tracts, areas with higher education and higher senior populations tend to have a higher propeortion of properties with a higher percentage of false positives, indicating missed exemption opportunities. People in those neighborhoods are more likely to get flagged by the model. This could reflect true differences in eligibility. For example, higher education levels may indicate more college students who do not own their homes, while areas with large senior populations may potentially reflect lower access and awareness of available exemptions.

# Biasness Check of Model
The false negative rate was analyzed across different demographic groups. As discussed earlier, false negatives refer to properties already enrolled in the Homestead Exemption program that the model incorrectly predicted as ineligible.

The model shows higher false negative rates—and thus lower effectiveness—in two types of areas:
...
These results suggest that while the model performs equitably for most groups, additional attention may be needed to ensure residents are not overlooked in outreach efforts.
