...
residential_properties <- residential_properties %>%
  left_join(residential_summary %>%
              select(parcel_number,avg_market_value,sd_market_value), 
...
residential_properties <- residential_properties %>%
  left_join(
    residential_transfers %>%
      select(objectid.x, document_type),
...
residential_properties <- residential_properties %>%
  left_join(
    minimal_financial_transfers_tocombine %>%
      select(objectid.x, min_fin_consid),
    by = c("objectid" = "objectid.x")
  )
```
...
residential_properties <- residential_properties %>%
  left_join(
    residential_transfers_2y %>%
      select(parcel_number, has_recent_transfer),
    by = c("parcel_number" = "parcel_number")
  )
```
...
```{r}
# joining census tract data to dataset

# Convert census tract to 6-digit format
residential_properties <- residential_properties %>%
  mutate(
    census_tract_fips = paste0("42101", str_pad(as.character(census_tract * 100), 6, pad = "0"))
  )

residential_properties <- residential_properties %>%
  left_join(census_tracts_enriched, by = c("census_tract_fips" = "GEOID"))

table(residential_properties$census_tract_fips %in% census_tracts_enriched$GEOID)

census_tracts_enriched$ALAND20
head(census_tracts_enriched$GEOID)
head(residential_properties$census_tract_fips)
table(residential_properties$census_tract)
```
...
```{r}
properties_model <- residential_properties %>% select(objectid, census_tract, geographic_ward, zip_code, geom, exemption,
                                                      ...
                                                      owner_occ_rate, poverty_rate, bach_degree_rate,
