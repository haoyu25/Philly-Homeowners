...
residential_properties <- residential_properties %>%
  left_join(
    residential_transfers %>%
      dplyr::select(objectid.x, document_type),
    by = c("objectid" = "objectid.x")
  )
```
...
residential_properties <- residential_properties %>%
  left_join(
    minimal_financial_transfers_tocombine %>%
      dplyr::select(objectid.x, min_fin_consid),
    by = c("objectid" = "objectid.x")
  )
```
...
residential_properties <- residential_properties %>%
  left_join(
    residential_transfers_2y %>%
      dplyr::select(parcel_number, has_recent_transfer),
    by = c("parcel_number" = "parcel_number")
  )
```
...
properties_model <- residential_properties %>% dplyr::select(objectid, census_tract, geographic_ward, zip_code, 
                                                      ...
                                                      same_address, potential_otherprog, is_deep, large_area,
                                                      rental_license, commercial_license, 
                                                      balance_total, balance_rate,
                                                      avg_market_value, sd_market_value,
                                                      document_type, min_fin_consid, has_recent_transfer,
