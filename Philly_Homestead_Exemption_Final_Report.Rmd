...
```
The dataset was filtered to include only properties that are under residential zoning codes. Properties with owners that are organizations and companies, such as owner names containing 'non profit' or 'office' or 'church', were also removed.
...
properties_model <- fread("Data/xgb_full_pred_0505.csv")
...
data1 <- read.csv("Data/xgb_full_pred_0505.csv")
census_tracts <- st_read("Data/phila_census1.gpkg")
...
fp_data <- read_csv("Data/fp_data_0505.csv")
...
![](rmd_png/outreachcost.png)

```{r outreach_cost, eval=FALSE}
costs <- data.frame(
  Category = c(
...
![](rmd_png/simple_filter.png)

```{r simple_filter, eval=FALSE}
