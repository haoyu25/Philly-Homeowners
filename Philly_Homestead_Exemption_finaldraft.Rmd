...
ggplot(residential_properties%>%filter(!is.na(is_deep)), 
       aes(x = as.factor(is_deep), fill = as.factor(exemption))) +
  geom_bar(position = "dodge") +
  scale_fill_manual(values = colors2, labels = c("No Exemption", "With Exemption")) +
  labs(
    title = "Property Depth by Homestead Exemption",
    x = "Depth > 300",
    y = "Count",
    fill = "Homestead Exemption"
  ) +
...
philly_shapefile <- st_read("data/tl_2020_42101_tract20/tl_2020_42101_tract20.shp")
```

## Spatial Density of Missed Exemptions

Kernel Density Estimation heatmap hotspots = Regions where the estimated probability density function is high.
...
```{r kde, echo=FALSE, message=FALSE, warning=FALSE, fig.show='hold'}
ggplot() +
